...
export default function ChurchHero({ 
  ...
  // Use provided title and welcomeText if available
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveIndex((prevIndex) => (prevIndex + 1) % processedSlides.length);
    }, 5000);
    ...
    <div className="hero-section mt-5 pt-3">
      <div id="churchCarousel" className="carousel slide" data-bs-ride="carousel">
        ...
        <button 
          ...
          type="button" 
          onClick={() => setActiveIndex(prev => (prev - 1 + processedSlides.length) % processedSlides.length)}
        >
        ...
        <button 
          ...
          type="button" 
          onClick={() => setActiveIndex(prev => (prev + 1) % processedSlides.length)}
        >
